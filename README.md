...
- The Claude hook set needs Claude Code ≥ 2.1.101: before that release an unknown hook event made Claude ignore the entire settings.json, so `tap install` refuses older versions rather than risk the user's config.
